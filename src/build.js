...
export function buildFiles(notebook, source, options = {}) {
  ...
  for (const [name, entry] of sharedFiles({ src, engine, engineVersion })) files.set(name, entry);
...
/**
 * The files that belong to the SITE rather than to any one page.
 *
 * Split out because a book's contents page needs the stylesheet too, and a book
 * may legitimately have no chapters yet — so these cannot be a by-product of
 * building one. isShared() names the same set from the other direction.
 */
export function sharedFiles({
  src = new URL('./', import.meta.url), engine = ENGINE_HOME, engineVersion = ENGINE_VERSION,
} = {}) {
  const files = new Map();
